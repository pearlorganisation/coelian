  ...
  return (
    <div>
      <li className="relative list-none ">
        <img onClick={() => { setDropDown((prevState) => !prevState) }} className="h-10" src={langFlag?.flag} alt="currentLangImg" />
        {
          dropDown &&
          <div className="absolute flex my-2 flex-col gap-3">
            {
              ...
              langFlagData.filter((item) => item.lang !== langFlag.lang).map((item, index) => {
                return <div key={index} onClick={() => { setLangFlag(item); setWebLang(item); setDropDown((prevState) => !prevState); }}><img className="h-10 " src={item?.flag} alt={item.lang} fill={"true"} /></div>
              })
            }
          </div>}
      </li></div>
