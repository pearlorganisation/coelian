...
// -------------------------------------------------States-----------------------------------------------    
  ...
  return (
    ...
    <div className="absolute flex my-2 flex-col gap-3">
    {
      ...
      langFlagData.filter((item)=> item.lang !== langFlag.lang).map(item => {
        return <div  onClick={()=>{ setLangFlag(item); setWebLang(item); setDropDown((prevState)=> !prevState); } }><img className="h-10 " src={item?.flag} alt={item.lang} /></div>
      })
    }
    </div>}
  </li></div>
